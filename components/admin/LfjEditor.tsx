...
export default function LfjEditor() {
  ...
  // Using the same document as lfjkids page for consistency
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage({ type: 'success', message: 'Pengaturan LFJ disimpan!' });
      router.refresh();
    } catch (error) {
  ...
  return (
    <form onSubmit={handleSave} className="space-y-6">
      ...
      <div>
        <label className="block text-sm font-medium text-gray-700 mb-1">Teks di Cloud/Bubble</label>
        <textarea 
          name="cloudText" 
          value={data.cloudText} 
          onChange={handleChange} 
          rows={3} 
          className={inputClass} 
          placeholder="Misal: Join us every&#10;Sunday 10am"
        />
      </div>

      <div className="border-2 border-dashed border-gray-300 p-4 rounded-lg">
        <label className="block text-sm font-medium text-gray-700 mb-2">Unggah Foto LFJ</label>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mt-4">
          {data.imageUrls.map((url, index) => (
            <div key={index} className="relative group">
              <img src={url} alt={`LFJ ${index}`} className="w-full h-32 object-cover rounded-md shadow-sm" />
              <button
                type="button"
