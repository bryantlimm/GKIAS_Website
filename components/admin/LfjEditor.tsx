// components/admin/LfjEditor.tsx
'use client';
...
interface LfjEditorData {
  imageUrls: string[];
  cloudText: string;
}

const initialData: LfjEditorData = {
  imageUrls: [],
...
export default function LfjEditor() {
  const router = useRouter();
  const inputRef = useRef<HTMLInputElement>(null);

  const [data, setData] = useState<LfjEditorData>(initialData);
  ...
  const sectionDocRef = doc(db, 'settings', 'lfjkids');
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        console.error('Error fetching LFJ data:', error);
      } finally {
        setLoading(false);
      }
    };
  ...
  const uploadFiles = async (files: FileList) => {
    ...
    try {
      ...
      for (let i = 0; i < files.length; i++) {
        const file = files[i];
        const storageRef = ref(storage, `lfj_images/${Date.now()}_${file.name}`);
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setStatusMessage({ type: 'success', message: 'Pengaturan LFJ berhasil disimpan!' });
      router.refresh();
    } catch (error) {
  ...
  if (loading) return <div style={{ color: '#94a3b8', fontSize: 14, padding: '24px 0' }}>Memuat data LFJ...</div>;
  ...
  return (
    <form onSubmit={handleSave} style={{ fontFamily: "'Nunito', sans-serif" }}>
      ...
      <div style={{ marginBottom: 20 }}>
        <FieldLabel>Teks di Cloud / Bubble</FieldLabel>
        <textarea
          name="cloudText" value={data.cloudText} onChange={handleChange}
          rows={3} placeholder={'Misal: Join us every\nSunday 10am'}
          style={{ ...inputStyle, resize: 'vertical' }} onFocus={onFocus} onBlur={onBlur}
        />
        <p style={{ margin: '5px 0 0', fontSize: 12, color: '#94a3b8' }}>
          Gunakan baris baru untuk memisahkan baris teks.
        </p>
      </div>

      {/* Image upload */}
      <div style={{ marginBottom: 28 }}>
        <FieldLabel>Foto LFJ <span style={{ fontWeight: 400, textTransform: 'none', letterSpacing: 0, color: '#94a3b8', fontSize: 11 }}>(bisa lebih dari 1)</span></FieldLabel>
        ...
        >
          ...
          <p style={{ margin: 0, fontSize: 13, fontWeight: 600, color: isUploading ? '#3b5bdb' : '#64748b' }}>
            {isUploading ? 'Mengunggah...' : 'Klik atau drag & drop foto LFJ'}
          </p>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(120px, 1fr))', gap: 10 }}>
            {data.imageUrls.map((url, i) => (
              <div key={i} style={{ position: 'relative', borderRadius: 8, overflow: 'hidden', aspectRatio: '1', background: '#f1f5f9' }}>
                <img src={url} alt={`LFJ ${i + 1}`} style={{ width: '100%', height: '100%', objectFit: 'cover', display: 'block' }} />
                <button
                  type="button"
